...
@Component({
  selector: 'app-program-newsfeed-content-card',
  template: `
    <div class="card flex">
      <div
...
export class ProgramNewsFeedContentCardComponent implements OnInit {
  @Input() cardTitle = ''
  @Input() iconUrl = ''
  @Input() number: number
  @Input() type: string
  ...
  async deleteCard() {
    this.newsFeedsClient.newsfeedsDelete(this.id.toString())
    await Promise.all([
      this.newsFeedsClient
        .newsfeedsDelete(this.id.toString())
        .pipe(
          tap(() => {
